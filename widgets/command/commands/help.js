...
const commandcfg = require("../command-config");

const myId = commandcfg.ownerIds[0];
const mainChannelId = commandcfg.mainChannelId;
const welcomeChannelId = commandcfg.welcomeChannelId;
...
module.exports = new CommandBuilder()
  ...
  .setExecute(async (message, user, args) => {
    ...
    if (args && args.length > 0) {
      const helpme = message.getHelp(args[0]);
      if (helpme.isHelpable) return await helpme.help();
    }

    const atme = await message.client.users.fetch(myId);
    const mainCh = message.client.channels.fetch(mainChannelId);
    const welcomeCh = message.client.channels.fetch(welcomeChannelId);
    ...
    const quoteBlock =
      ...
      `There *should* be a post about it in ${welcomeCh}, eventually... \n`;

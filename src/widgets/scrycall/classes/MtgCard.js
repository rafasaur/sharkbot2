...
class MtgCard {
  ...
  fuzzyQuery() {
    return fetch(this.apiUrl + 'named?fuzzy=' + this.searchName)
      .then(response => response.json())
      .catch(err => console.log(err));
  }

  searchQuery(addTerms) {
    return fetch(this.apiUrl + 'search?q=' + this.searchName + addTerms)
      .then(response => response.json())
      .catch(err => console.log(err));
  }
  ...
  async send(channel) {
    ...
    switch (this.sendType) {
      case 'error':
        channel.send({content:this.sendData});
        break;
      case 'list':
        channel.send({content:this.listCards(this.sendData)});
        break;
      case 'card':
        this.makeCard(this.sendData)
          .then( async cardSides => {
            ...
            if (cardSides.length === 1) {
              channel.send({embeds: [new MessageEmbed(cardSides[0])]})//.then( msg => {
              //  setTimeout( () => {

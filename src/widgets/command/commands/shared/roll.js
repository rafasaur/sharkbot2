...
const numberedRoll = (rolls, emojiCache) => {
  let replies = [];
  rolls.forEach( roll => {
    let sum = 0;
    let rollList = [];
    ...
    if (sum === 100) sum = '💯';
    else if (sum === 110) sum = emojiCache.find(emoji => emoji.name === '110') || 110;
    else if (sum === 69) sum = 'NICE';
    else if (sum === 420) sum = emojiCache.find(emoji => emoji.name === 'weednumber') || 'blaze it';
...
module.exports = ( rollString, emojiCache ) => {

  const numberedRolls = rollString.match(/(\d*d\d+)/g);
  const destinyRolls = rollString.match(/(\d*d((red|purple|black|blue|green|yellow|white)|[rpkugyw]))/g);

  let numReplies = [];
  let swReply = "";
  if (numberedRolls) numReplies = numberedRoll(numberedRolls, emojiCache);
  if (destinyRolls) swReply = destinyRoll(destinyRolls);

  return numReplies.concat([swReply]);
}
